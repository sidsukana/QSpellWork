...
namespace SpellRange
{
    struct entry
    {
        ...
        float       maxRange;                                        // 2
        quint32     flags;                                           // 3
        quint32     nameOffset[16];                                  // 4-19 unused
        quint32     nameFlags;                                       // 20 unused
        quint32     shortNameOffset[16];                             // 21-36 unused
...
namespace Spell
{
    ...
    struct entry
    {
        ...
        quint32    nameOffset[16];                                  // 127-142
        quint32    nameFlag;                                        // 143
        quint32    rankOffset[16];                                  // 144-159
        quint32    rankFlags;                                       // 160
        quint32    descriptionOffset[16];                           // 161-176 not used
        quint32    descriptionFlags;                                // 177     not used
        quint32    toolTipOffset[16];                               // 178-193 not used
    ...
    class meta : public QObject
    {
        ...
        {
            for (quint8 i = 0; i < MAX_EFFECT_INDEX; ++i)
                return (m_info->effectApplyAuraName[i] == auraId);
            return false;
        }
        ...
        {
            for (quint8 i = 0; i < MAX_EFFECT_INDEX; ++i)
                return (m_info->effect[i] == effectId);
            return false;
        }
        ...
        {
            for (quint8 i = 0; i < MAX_EFFECT_INDEX; ++i)
                return (m_info->effectImplicitTargetA[i] == targetId);
            return false;
        }
        ...
        {
            for (quint8 i = 0; i < MAX_EFFECT_INDEX; ++i)
                return (m_info->effectImplicitTargetB[i] == targetId);
            return false;
        }
